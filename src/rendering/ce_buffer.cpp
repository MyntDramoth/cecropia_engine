...
CE_Buffer::~CE_Buffer() {
    unmap();
    vkDestroyBuffer(ce_device.device(), buffer, nullptr);
    vkFreeMemory(ce_device.device(), memory, nullptr);
}
...
VkResult CE_Buffer::map(VkDeviceSize size, VkDeviceSize offset) {
    ...
    return vkMapMemory(ce_device.device(), memory, offset, size, 0, &mapped);
}
...
void CE_Buffer::unmap() {
    if (mapped) {
        vkUnmapMemory(ce_device.device(), memory);
        mapped = nullptr;
    }
}
...
VkResult CE_Buffer::flush(VkDeviceSize size, VkDeviceSize offset) {
    ...
    return vkFlushMappedMemoryRanges(ce_device.device(), 1, &mappedRange);
}
...
VkResult CE_Buffer::invalidate(VkDeviceSize size, VkDeviceSize offset) {
    ...
    mappedRange.offset = offset;
    mappedRange.size = size;
    return vkInvalidateMappedMemoryRanges(ce_device.device(), 1, &mappedRange);
}
 ...
 *
 */
VkResult CE_Buffer::flushIndex(int index) { return flush(alignmentSize, index * alignmentSize); }
