...
#define ENGINE_DIR "../"
 ...
 std::vector<char> Render_Pipeline::read_file(const std::string &file_path) {
    std::string engine_path = ENGINE_DIR + file_path;
    std::ifstream file{ engine_path, std::ios::ate | std::ios::binary}; //ate goes to the end of the file imediately
    if (!file.is_open()) {
...
Render_Pipeline::Render_Pipeline(
    ...
    const pipeline_config_info& config) : ce_device{device} {
    create_render_pipeline(vert_path, frag_path, config);
}

Render_Pipeline::~Render_Pipeline() {
   vkDestroyShaderModule(ce_device.device(),vert_shader_module,nullptr);
   vkDestroyShaderModule(ce_device.device(), frag_shader_module, nullptr);
   vkDestroyPipeline(ce_device.device(), graphics_pipeline, nullptr);
}

void Render_Pipeline::bind(VkCommandBuffer command_buffer) {
    vkCmdBindPipeline(command_buffer,VK_PIPELINE_BIND_POINT_GRAPHICS,graphics_pipeline);
}


void Render_Pipeline::create_render_pipeline(const std::string& vert_path, const std::string& frag_path, const pipeline_config_info& config) {
    
    assert(config.pipeline_layout != VK_NULL_HANDLE && "Cannot create graphics pipeline info:: no pipeline_layout in pipeline_config_info");
    assert(config.render_pass != VK_NULL_HANDLE && "Cannot create graphics pipeline info:: no render_pass in pipeline_config_info");
    ...
    auto& binding_descritions = config.binding_descriptions;
    auto& attribute_descriptions = config.attribute_descriptions;

    VkPipelineVertexInputStateCreateInfo vertex_input_info{};
    vertex_input_info.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
    vertex_input_info.vertexAttributeDescriptionCount = static_cast<uint32_t>(attribute_descriptions.size());
    vertex_input_info.vertexBindingDescriptionCount = static_cast<uint32_t>(binding_descritions.size());
    vertex_input_info.pVertexAttributeDescriptions = attribute_descriptions.data();
    vertex_input_info.pVertexBindingDescriptions = binding_descritions.data();
    ...
    if (vkCreateGraphicsPipelines(ce_device.device(), VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &graphics_pipeline) != VK_SUCCESS) {
        throw std::runtime_error("failed to create graphics pipeline");
    }
}


void Render_Pipeline::create_shader_module(const std::vector<char>& shader_code, VkShaderModule* shader_module) {
    VkShaderModuleCreateInfo create_info{};
    create_info.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
    create_info.codeSize = shader_code.size();
    create_info.pCode = reinterpret_cast<const uint32_t*>(shader_code.data());

    if (vkCreateShaderModule(ce_device.device(), &create_info, nullptr, shader_module) != VK_SUCCESS) {
        throw std::runtime_error("failed to create shader module");
    }
}

void Render_Pipeline::default_pipeline_config(pipeline_config_info& config_info) {
    ...
    config_info.binding_descriptions = CE_Model::Vertex::get_binding_descriptions();
    config_info.attribute_descriptions = CE_Model::Vertex::get_attribute_descriptions();

}

void Render_Pipeline::enable_alpha_blending(pipeline_config_info& config_info) {
    //////////////////////
    //color_blend_attachment
    //////////////////////
    config_info.color_blend_attachment.colorWriteMask =
